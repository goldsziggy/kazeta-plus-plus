...
pub fn nsstring_to_string(string: ObjcId) -> String {
    unsafe {
        // On some macOS setups (e.g., running without a full app bundle),
        // Cocoa APIs can return nil here. Guard against null to avoid
        // dereferencing and aborting before the app even starts.
        if string.is_null() {
            return String::new();
        }
